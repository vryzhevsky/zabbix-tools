...
class TelegramAPI:
    ...
    def send_message(self, to, message):
        ...
        message = "\n".join(message).replace("<html>","").replace("</html>","")
...
class ZabbixWeb:
    ...
    def graph_get(self, itemid, period, title, width, height):
        ...
        zbx_img_url = self.server + "/chart3.php?from=now-{0}&to=now&period={0}&name={1}" \
                                    "&width={2}&height={3}&graphtype=0&legend=1" \
                                    "&yaxismin=0&&ymin_type=1".format(period, title, width, height)
    ...
    def get_triggers_active(self, priority=[4,5]):
        headers = {'Content-type': 'application/json'}
        method = "trigger.get"
        params = {
            #"groupids": zbxtg_settings.zbx_hostgroup_ids,
...
def main():
    ...
    settings = {
        ...
        "zbxtg_image_age": "1h",
        "zbxtg_image_width": "384",
        "zbxtg_image_height": "108",
    ...
    if zbxtg_settings.proxy_to_tg:
        ...
        if not proxy_to_tg.find("http") and not proxy_to_tg.find("socks"):
            proxy_to_tg = "socks5://" + proxy_to_tg
        #proxy_to_tg = "socks5://" + proxy_to_tg
        #print("Proxy to tg: %s" % proxy_to_tg)
        tg.proxies = {
    ...
    if not is_single_message:
        ...
        if not tg.ok:
            ...
            # another case if html is enabled and we got parse error, try to remove "bad" symbols from message
            if tg.html and tg.error.find("Unsupported start tag") > -1:
                html_fix_attempts = 0
                while not tg.ok and html_fix_attempts != 10:
                    offset = re.search("Unsupported start tag .* at byte offset ([0-9]+)", tg.error).group(1)
                    zbxtg_body_text = html_fix(zbxtg_body_text, offset)
                    tg.send_message(uid, zbxtg_body_text)
                    html_fix_attempts += 1
                if tg.ok:
                    print_message("Original message has been fixed due to html parse bugs")
